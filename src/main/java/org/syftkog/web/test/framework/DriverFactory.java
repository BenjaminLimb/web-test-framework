...
public class DriverFactory implements HasDriver {
    ...
    private final static String GECKO_DRIVER_PATH = PropertiesRetriever.getString("webdriver.gecko.driver", null);
    ...
    private Driver getNewLocalDriver(Capabilities capabilities) {
        ...
        if (browser.equalsIgnoreCase("firefox")) {
            ...
            try {
                ...
                if (ex.getMessage().contains("Cannot find firefox binary in PATH")) {
                    throw new RuntimeException("Firefox must be installed in order to run unit tests.", ex);
                } else {
                    throw ex;
                }
            }
...
//        sauceSupportedCapabilities.setCapability("idle-timeout",sauceIdleTimeout);
    ...
    public Driver getDriver() {
        Driver driver = getDriver(BrowserVersionPlatform.MOCK);
        return driver;
    }
